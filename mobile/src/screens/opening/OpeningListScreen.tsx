...
export function OpeningListScreen({ navigation }: Props) {
  ...
  // Which row's settings sheet is open — it offers that row's openings, and nothing else.
  const [settingsSide, setSettingsSide] = useState<'w' | 'b' | null>(null);

  // An opening curated for both sides sits in both rows — same lines, opened from the other chair.
  const bySide = useMemo(
    () => ({
      w: OPENINGS.filter((opening) => learnSides(opening).includes('w')),
      b: OPENINGS.filter((opening) => learnSides(opening).includes('b')),
    }),
    []
  );

  const renderRow = (side: 'w' | 'b', title: string, subtitle: string) => {
    const shown = bySide[side].filter((opening) => isVisible(opening.id));
    return (
      <View style={styles.section}>
        {/* The row's own ⚙ picks what that row shows — one button per row rather than one per card. */}
        <View style={styles.sectionHeader}>
          <View style={[styles.sideDot, side === 'b' && styles.sideDotBlack]} />
          <Text style={styles.sectionTitle}>{title}</Text>
          <Text style={styles.sectionSubtitle}>{subtitle}</Text>
          <View style={styles.sectionSpacer} />
          <Pressable onPress={() => setSettingsSide(side)} hitSlop={10} style={styles.sectionSettings}>
            <Ionicons name="settings-outline" size={20} color={colors.textMuted} />
          </Pressable>
        </View>
        {shown.length === 0 ? (
          <Pressable style={styles.emptyRow} onPress={() => setSettingsSide(side)}>
            <Text style={styles.emptyRowText}>표시할 오프닝이 없습니다 — 오른쪽 ⚙ 에서 골라 주세요</Text>
          </Pressable>
        ) : (
          <ScrollView
            horizontal
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <OpeningVisibilityModal
        visible={settingsSide !== null}
        onClose={() => setSettingsSide(null)}
        openings={settingsSide ? bySide[settingsSide] : []}
        title={settingsSide === 'b' ? '흑 오프닝 고르기' : '백 오프닝 고르기'}
      />
    </SafeAreaView>
  );
}
