...
값이 틀리면 분석 화면에 **"분석 서버에 연결할 수 없습니다"**가 뜹니다(`api/analysisApi.ts`). `EXPO_PUBLIC_*`는 **번들 타임에 인라인**되므로 `.env`를 고친 뒤에는 반드시 캐시를 비우고 다시 띄워야 합니다:
...
백엔드를 WSL2 안에서 돌리면 기본 NAT 모드의 WSL IP(`172.x`)는 폰에서 닿지 않습니다. Windows 쪽 `%USERPROFILE%\.wslconfig`에 미러 모드를 켜면 WSL이 Windows의 네트워크 인터페이스를 그대로 공유해, PC의 Wi-Fi IP 하나로 백엔드(8080)·Metro(8081)에 전부 접근할 수 있습니다.
...
바꾼 뒤 `wsl --shutdown`으로 재시작하고, `hostname -I`로 나온 주소를 `.env`에 넣으세요. 문제가 생기면 `networkingMode` 줄만 지우면 원래대로 돌아갑니다.

미러 모드에서 주의할 점 두 가지:

* **Windows에서 WSL 서비스를 부를 때는 `localhost` 대신 `127.0.0.1`을 쓰세요.** `localhost`가 IPv6 `::1`로 먼저 해석되는데 그쪽은 연결되지 않습니다.
* 폰에서 접속이 안 되면 Windows 방화벽입니다. 관리자 PowerShell에서 인바운드를 열어 주세요:

  ```powershell
  New-NetFirewallRule -DisplayName "Chessko dev (8080,8081)" -Direction Inbound -Protocol TCP -LocalPort 8080,8081 -Action Allow
  ```

DHCP로 받은 주소라 공유기를 재부팅하거나 다른 네트워크로 옮기면 IP가 바뀝니다. 그때는 `.env`를 고치고 Metro를 다시 띄우면 됩니다.

### 라인 등록기 주소 (선택)

```bash
# mobile/.env
EXPO_PUBLIC_AUTHOR_URL=http://127.0.0.1:4599
```

`💬 유저 설명` 칸의 **라인 등록기 열기 ↗** 버튼이 열 주소입니다. `npm run author`로 띄우는 로컬 편집기를 가리키며, 기여 사이트가 생기면 이 값만 그쪽으로 돌리면 됩니다.

**설정하지 않으면 버튼 자체가 렌더링되지 않습니다**(`components/CommunityNotes.tsx`). 배포 빌드에서 개발 PC의 주소를 가리키는 링크가 사용자에게 보이지 않게 하려는 의도이고, `.env`가 EAS 빌드에 올라가지 않으므로 릴리스 APK에서는 자동으로 숨겨집니다.
...
| 프로필 | 산출물 | OTA 채널 | 용도 |
| --- | --- | --- | --- |
| `development` | APK (dev client) | `development` | 네이티브 모듈 디버깅 |
| `preview` | APK | `preview` | 기기에 바로 설치해 확인 |
| `production` | AAB | `production` | 플레이 콘솔 업로드 |
| `production-galaxy` | APK | `production-galaxy` | 갤럭시 스토어용 — **현재 미사용**(삼성이 무료 앱에도 사업자등록을 요구해 보류) |
...
설정하지 않으면 `http://localhost:8080`으로 떨어져 **분석·복기·스캔만 실패**하고, 오프닝 학습·퍼즐·연습은 정상 동작합니다. 다만 테스터에게 돌릴 빌드라면 **백엔드를 먼저 띄우고 이 값을 넣은 뒤 빌드하세요** — 죽은 기능을 준 채로 받는 피드백은 "이거 안 돼요"로 오염됩니다.

## OTA 업데이트 (EAS Update)

`expo-updates`가 설정돼 있어 **JS·에셋 수정은 스토어 심사 없이** 배포할 수 있습니다.

```bash
eas update --branch production --message "오프닝 목록 스크롤 버그 수정"
```

같은 채널로 만든 빌드가 **다음 실행 때** 자동으로 받습니다. 제약 두 가지:

* **JS·에셋만** 갱신됩니다. 새 패키지·권한·아이콘/스플래시 같은 **네이티브 변경은 새 빌드**가 필요합니다.
* `runtimeVersion` 정책이 `appVersion`이라 **`app.json`의 `version`이 같은 빌드끼리만** 업데이트가 적용됩니다. 테스트 기간에는 `version`을 고정하고 JS 수정만 OTA로 미세요 — 올리는 순간 기존 설치본은 업데이트를 못 받습니다.
...
│   └── AnalysisScreen         FEN 입력 · 스크린샷 스캔 · 엔진 분석 + 이밸류에이션 바 (백엔드 필요)
...
보드 옆 이밸류에이션 바는 **화면에 들어온 순간부터 계속 붙어 있습니다.** 분석 전에는 반반으로 그려지고 라벨이 `—`이며, 분석이 끝나면 그 자리에서 값만 채워집니다. 결과가 도착할 때 레이아웃이 흔들리지 않게 하려는 것이라, 바를 조건부로 렌더링하지 마세요.

바에 넣는 값은 rank 1 라인의 평가값을 `whitePerspectiveCp()`로 백 기준으로 변환한 것입니다. **엔진은 둘 차례인 쪽 기준으로 점수를 주기 때문에 매 수마다 부호가 뒤집힙니다** — 이 변환을 건너뛰면 흑 차례에서 우열이 반대로 표시됩니다.

추천 수 3개는 엔진이 주는 UCI(`e7e5`)가 아니라 **SAN(`e5`, `Nf3`, `exd5`, `O-O`, `a8=Q+`)으로 표시**합니다. 변환은 표시 직전에 `AnalysisPanel`이 응답의 `fen`을 써서 합니다. 백엔드 응답 자체는 UCI로 두는데, `ExploreMode`가 `bestMove`를 `from`/`to`로 잘라 보드에 두기 때문입니다.

포지션이 바뀌면(수를 두거나 FEN을 불러오거나 기보를 되짚으면) 분석 결과가 비워지고 바도 `—`로 돌아갑니다. 이전 포지션의 평가값이 남아 잘못된 정보를 주지 않게 하려는 의도입니다.
...
│   ├── EvalBar.tsx        체스닷컴식 세로 이밸류에이션 바 (대국 복기 · 포지션 분석 · 탐색 모드)
...
> ⚠️ **편집기에는 인증이 없고 모든 인터페이스(`*:4599`)에 열립니다.** 쓰기 엔드포인트 7개(`/api/note`, `/api/community`, `/api/branch`, `/api/opening`, `/api/rename`, `/api/delete`, `/api/position`)가 `src/data/*.json`에 바로 기록하므로, **같은 네트워크에 있는 누구나 오프닝을 수정하거나 삭제할 수 있습니다.** 공용 와이파이에서는 켜지 마세요. 필요하면 `scripts/author-server.mjs`의 `server.listen(PORT)`를 `server.listen(PORT, '127.0.0.1')`로 바꿔 루프백으로 잠글 수 있습니다 — PC 브라우저에서는 그대로 쓰고 네트워크에서만 차단됩니다.

편집기가 쓴 내용은 **저장소 소스 파일**에 들어갑니다. 개발 중인 앱에는 Metro 핫리로드로 즉시 반영되지만, 이미 설치된 사용자 앱에는 커밋 → 빌드/OTA를 거쳐야 나갑니다.
