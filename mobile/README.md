...
어떤 위치에서 **"＋ 새 갈래 추가"** 를 누르면, 그 지점부터 보드에서 직접 수를 두어 **새 상대 응수 라인**을 만들고 이름·종류를 붙여 저장합니다. 저장 대상은 `data/openingBranches.json`(키: `openingId` → `OpeningLine[]`)이고, `openingsRuntime.ts`가 이걸 큐레이션 라인에 **병합**하므로 앱 트리에 즉시 나타납니다 — 사용자가 그 수를 두면 큐레이션 변형과 똑같이 라인 이름이 뜨고 학습됩니다. 저장 후 그 라인을 골라 설명을 답니다. (모든 수는 저장 시 chess.js로 합법성 검증)
