...
* **오프닝 탭** — 백/흑 오프닝을 가로 스크롤 캐러셀에서 고르고, 보드 위 화살표를 따라 수순을 한 수씩 익힙니다. 수순을 벗어난 수를 두면 화면 테두리가 빨갛게 번쩍이며 자동으로 되돌아갑니다. 아무 포지션에서나 "이 포지션 분석하기"로 엔진 분석 화면에 진입할 수 있습니다.
* **연습 탭** — 좌표 연습(리체스 스타일), 메이트 퍼즐, 저장한 오프닝의 다음 수를 맞히는 리콜 퀴즈.
* **마이페이지 탭** — chess.com 닉네임 연동(공개 API, 인증 불필요)으로 최근 대국을 확인하고, 로컬 학습 통계를 한눈에 봅니다.
...
| Study | 오프닝 학습 | 큐레이션한 오프닝 20개를 화살표 가이드로 한 수씩 학습, 북마크 | `mobile` |
...
* **Phase 5 진행 중 — Lotus 스타일 앱 개편 (`mobile/`)** — 단일 FEN 분석기 화면을 하단 3탭 구조(오프닝 / 연습 / 마이페이지)로 재구성. 오프닝 캐러셀 + 화살표 가이드, 좌표 연습, 메이트 퍼즐 30개까지 완료. 오프닝 리콜 퀴즈와 chess.com 연동이 남아 있습니다.
